...
#' Dummy function
#'
#' Simple dummy function to check that R and Rcpp are talking to each other.
#'
#' @export
...
human_equilibrium_noHet <- function(EIR, ft, p, age) {
    ...
    # produce population age distribution using eta, which is defined as 1/average age. The population distribution can be envisioned as an exponential distribution, with mass feeding in from the left due to birth at rate eta, people ageing with rates defined based on the width of the age groups, and mass leaking out of all categories with death rate eta (making the distribution stable).
	for(i in 1:na){
	...
	for(i in 1:na){
        ...
        # flow out of this category, i.e. rate of ageing plus death
        re <- r[i] + p$eta
    ...
    for(i in 2:na) {
        ICM[i] <- (r[i] + p$eta)*ICM[i-1]/(1/p$dm+r[i] + p$eta)
    }
