...
//------------------------------------------------
// matrix class from linear vector
class mat_double {
private:
  // elements
  int rows;
...
public:
  // constructors
  mat_double() {};
  mat_double(int rows, int cols) {
    this->rows = rows;
    this->cols = cols;
    x = std::vector<double>(rows*cols);
  }
  
  // methods
  double get(int i, int j) {return x[i*cols+j];}
  void set(int i, int j, double y) {x[i*cols+j] = y;}
